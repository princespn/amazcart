...
trait Otp
{
    ...
    public function sendOtp($request, $type = Null)
    {
        ...
        if (is_numeric($request->email)) {
            $smsTemplete = SmsTemplate::where('type_id', 19)->where('is_active', 1)->first(); //registration templete
            $msg = $smsTemplete->value;
            try{
                return $this->sendSMS($request->email, $msg,'','','',$code);
            }catch(Exception $e){}
    ...
    public function sendLoginOtp($request, $type = Null)
    {
        ...
        if (is_numeric($request->login)) {
            ...
            $msg = $smsTemplete->value;
            try{
                return $this->sendSMS($request->login, $msg,'','','',$code);
            }catch(Exception $e){}
    ...
    public function sendOtpForSeller($request, $type = Null)
    {
        ...
        if (Str::contains(otp_configuration('otp_type_registration'), 'sms')) {
            ...
            $msg = $smsTemplete->value;
            try{
                $smsSend = $this->sendSMS($request->phone, $msg,'','','',$code);
            }catch(Exception $e){}
        }
